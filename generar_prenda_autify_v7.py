...
def add_spaces(s):
    if not s: return s
    s=re.sub(r'(AV|DE|SAN|LAS|LOS|EL|LA|DR|GEN|BRIG|ALTE|CNEL|ING)(?=[A-ZÁÉÍÓÚÑ])',r'\1 ',s)
    s=re.sub(r'([a-záéíóúñ])([A-ZÁÉÍÓÚÑ])',r'\1 \2',s)
    # Inserta espacio entre letra y número pegados (ej: "CALLE65" -> "CALLE 65")
...
def parsear_solicitud(pdf_path):
    ...
    nc=ex(t1,r'ApellidoyNombre:\s*([A-ZÁÉÍÓÚÑ ,]+?)Fecha')
    if ',' in nc:
...
def resolver(variable_id, d):
    ...
    # ── HOJA 2 ──────────────────────────────────────────────
        ...
        if d['tiene_conyuge']: add(24, 27, d['nombre_completo'])
        ...
        if d['tiene_conyuge']: add(61, 50, 'X', 14)
